...
export default function PreAnest() {
  ...
  const toggleComorbidity = (c: string) => {
    set(
      'comorbidities',
      input.comorbidities.includes(c)
      ...
      <Card className="clinical-shadow">
        <CardContent className="p-4 space-y-4">
          ...
          <div>
            ...
            <select
              value={input.asa}
              onChange={(e) => set('asa', +e.target.value as 1 | 2 | 3 | 4 | 5)}
              className="w-full mt-1 h-9 rounded-md border bg-background px-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30"
            >
              {[1, 2, 3, 4, 5].map((v) => (
          ...
          <div>
            ...
            <input
              type="text"
              value={input.otherComorbidities}
              onChange={(e) => set('otherComorbidities', e.target.value)}
              placeholder={t('other_comorbidities')}
              className="w-full mt-2 h-9 rounded-md border bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30"
            />
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select
                value={input.mallampati}
                onChange={(e) => set('mallampati', +e.target.value as 1 | 2 | 3 | 4)}
                className="w-full mt-1 h-9 rounded-md border bg-background px-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30"
              >
                {[1, 2, 3, 4].map((v) => (
                  <option key={v} value={v}>
            ...
            <div>
              ...
              <select
                value={input.mouthOpening}
                onChange={(e) => set('mouthOpening', e.target.value as 'normal' | 'limited')}
                className="w-full mt-1 h-9 rounded-md border bg-background px-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30"
              >
            ...
            <div>
              ...
              <select
                value={input.cervicalMobility}
                onChange={(e) => set('cervicalMobility', e.target.value as 'normal' | 'limited')}
                className="w-full mt-1 h-9 rounded-md border bg-background px-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30"
              >
                <option value="normal">{t('normal')}</option>
                <option value="limited">{t('limited')}</option>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => set('anticoagulation', e.target.value)}
              className="w-full mt-1 h-9 rounded-md border bg-background px-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30"
            >
              {ANTICOAG_OPTIONS.map((o) => (
          ...
          {blocks.map((block) => {
            ...
            return (
              <Card key={block.key} className="clinical-shadow overflow-hidden">
                <button
                  onClick={() => setExpandedBlock(isOpen ? null : block.key)}
