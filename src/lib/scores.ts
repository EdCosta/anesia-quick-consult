...
export function calcApfel(i: ApfelInputs): ScoreResult {
  const score = [i.femaleGender, i.nonSmoker, i.ponvHistory, i.postopOpioids].filter(
    Boolean,
  ).length;
...
export function calcCaprini(i: CapriniInputs): ScoreResult {
  const pts1 = [
    ...
    i.varicoseVeins,
    i.pregnancy,
    i.historyUnexplainedStillbirth,
    i.oralContraceptives,
    i.sepsis,
    i.seriousLungDisease,
    i.abnormalPulmonaryFunction,
    i.acuteMI,
    i.chf,
    i.medicalPatientBedRest,
    i.ibd,
  ].filter(Boolean).length;
  const pts2 =
    [
      ...
      i.bedRestOver72h,
      i.immobilizingCast,
      i.centralVenousAccess,
    ].filter(Boolean).length * 2;
  const pts3 =
    [
      i.ageOver75,
      i.historyDVTPE,
      i.familyHistoryThrombosis,
      i.factorVLeiden,
      i.prothrombinMutation,
      i.lupusAnticoagulant,
      i.anticardiolipinAntibodies,
      i.homocysteine,
      i.heparinInducedThrombocytopenia,
      i.otherThrombophilia,
    ].filter(Boolean).length * 3;
  const pts5 =
    [i.stroke, i.elHipKneeFx, i.multipleTrauma, i.acuteSpinalCordInjury].filter(Boolean).length * 5;
  const score = pts1 + pts2 + pts3 + pts5;
  const category =
    score <= 1 ? 'low' : score === 2 ? 'moderate' : score <= 4 ? 'high' : 'very_high';
