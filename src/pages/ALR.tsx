...
export default function ALR() {
  ...
  const fuse = useMemo(() => new Fuse(alrBlocks, { keys: [`titles.${lang}`, 'titles.fr', 'region'], threshold: 0.4, ignoreLocation: true }), [alrBlocks, lang]);
    ...
    <div className="container py-8 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
          <Target className="h-6 w-6 text-accent" />{t('alr_full')}
        </h1>
      ...
      <div className="relative max-w-md">
        ...
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t('search_alr')} className="h-10 w-full rounded-lg border border-border bg-card pl-10 pr-4 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/50" />
      </div>

      <div className="flex flex-wrap gap-2">
        <button onClick={() => setRegion(null)} className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${!region ? 'bg-accent text-accent-foreground border-accent' : 'bg-card text-muted-foreground border-border hover:border-accent/50'}`}>{t('all_regions')}</button>
        {regions.map((r) => (
          <button key={r} onClick={() => setRegion(r === region ? null : r)} className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${r === region ? 'bg-accent text-accent-foreground border-accent' : 'bg-card text-muted-foreground border-border hover:border-accent/50'}`}>
            ...
            {visibleItems.map((block) => {
              ...
              return (
                <div key={block.id} className="rounded-xl border bg-card clinical-shadow overflow-hidden">
                  <button onClick={() => setExpanded(isOpen ? null : block.id)} className="w-full flex items-center justify-between p-4 text-left hover:bg-muted/30 transition-colors">
                    <div>
                      <h3 className="text-sm font-semibold text-card-foreground">{resolveStr(block.titles)}</h3>
                      <Badge variant="secondary" className="mt-1 text-[11px]">{REGION_MAP[block.region] ? t(REGION_MAP[block.region]) : block.region}</Badge>
                    </div>
...
function ALRSection({ title, items, variant }: { title: string; items: string[]; variant?: 'danger' | 'info' }) {
  if (items.length === 0) return null;
  const bulletColor = variant === 'danger' ? 'text-clinical-danger' : variant === 'info' ? 'text-clinical-info' : 'text-accent';
  return (
    <div>
      <p className="text-xs font-semibold text-muted-foreground mb-1">{title}</p>
      <ul className="space-y-1">
        {items.map((item, i) => (
          <li key={i} className="text-xs text-card-foreground flex gap-2"><span className={`${bulletColor} mt-0.5`}>•</span><span>{item}</span></li>
        ))}
      </ul>
    </div>
  );
